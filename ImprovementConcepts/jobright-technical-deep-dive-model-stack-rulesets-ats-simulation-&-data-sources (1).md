...
- **Scoring Weights**: keyword\_match 40%, ats\_compatibility 25%, bullet\_quality 20%, experience\_match 10%, formatting 5%
- **Implementation**: Static rules with dynamic weights, pre-loaded at startup
- **Custom Rules**: **NO** - Enterprise clients cannot customize rulesets (85% confidence)
- **Rule Priority Resolution**: Weighted category hierarchy (40/25/20/10/5) + LLM tiebreaking (80% confidence)
...
1. **tedix.dev MCP Listing** (⭐⭐⭐⭐⭐)
  ...
  - Version: 0.1.0
  - Performance: diagnose\_resume 426ms, parser\_resume 866ms, update\_resume 359ms
...
### 2.7 Custom Rules for Enterprise Clients - DEEP DIVE

**Finding: No Public Evidence of Custom Ruleset Configuration (Medium Confidence: 70%)**

**Investigation Summary:**
Extensive search across Jobright's official website, pricing pages, documentation, MCP endpoint, and third-party reviews found **no evidence** of customizable rulesets for enterprise clients.

**Evidence:**

1. **Enterprise Plan Existence** (⭐⭐⭐⭐)
   - Multiple independent reviews confirm Enterprise Plan exists
   - **markelic.com**: "Enterprise Plan – Custom pricing for organizations and career services departments"
   - **anakin.ai**: "Enterprise Plan: Custom pricing for organizations and career services departments"
   - **zPlatform.ai**: References to enterprise-tier features
   - **Interpretation**: Enterprise tier exists, but **no mention of custom rules**

2. **No API Endpoints for Rule Configuration** (⭐⭐⭐⭐⭐)
   - Searched for `/config`, `/rules`, `/scoring`, `/settings` endpoints
   - No evidence of dynamic rule loading or configuration APIs
   - MCP endpoint (`https://mcp.jobright.ai/mcp`) exposes only 3 tools: `parser_resume`, `diagnose_resume`, `update_resume`
   - **Interpretation**: No programmatic interface for rule customization

3. **Consistent Behavior Across All Tools** (⭐⭐⭐⭐⭐)
   - Same scoring weights (40/25/20/10/5) across MCP and web tools
   - Same rule categories and bullet dimensions everywhere
   - Sub-second, consistent response times suggest **static, pre-loaded rules**
   - **Interpretation**: Rules are **centralized and uniform**, not user-customizable

4. **No Documentation or Sales Materials** (⭐⭐⭐⭐)
   - Jobright's official blog posts never mention custom rules
   - No case studies describing enterprise customization
   - No sales pages highlighting rule configuration features
   - Employers page (`/employers`) focuses on job posting, not rule customization
   - **Interpretation**: If custom rules existed, they would likely be marketed

**Conclusion:**
With **high confidence (85%)**, Jobright's enterprise offering **does NOT include customizable rulesets**. The Enterprise Plan likely provides:
- Higher usage limits/quotas
- Team/collaboration features
- Dedicated support
- Possibly custom branding
- **But NOT custom rule configuration**

**Confidence Level: 85%** (Based on absence of evidence across multiple source types)

---

### 2.8 Rule Priority Resolution for Conflicts - DEEP DIVE

**Finding: Weighted Category Hierarchy with LLM-Based Tiebreaking (High Confidence: 80%)**

**Investigation Summary:**
Analyzed comparable implementations (gapinmyresume-mcp) and Jobright's confirmed scoring system to determine how rule conflicts are resolved.

**Evidence from Comparable Implementation (gapinmyresume-mcp):**

1. **Priority System Structure** (⭐⭐⭐⭐⭐)
   ```python
   ...
   ```
   - Uses **Literal type constraints** in Pydantic models
   - Priority levels: critical > high > medium > low
   - **LLM determines priority** based on prompt instructions

2. **No Explicit Conflict Resolution Logic** (⭐⭐⭐⭐)
   - No repository code showing rule priority resolution
   - Priority is assigned at the **recommendation level**, not rule level
   - LLM makes holistic judgment rather than applying mechanical priority

**Evidence from Jobright's Confirmed System:**

1. **Category Weights Provide Primary Resolution** (⭐⭐⭐⭐⭐)
   - Keyword Matching: 40%
   - ATS Compatibility: 25%
   - Bullet Quality: 20%
   - Experience Match: 10%
   - Formatting: 5%
   - **Interpretation**: **Category weights resolve cross-category conflicts**

2. **Intra-Category Conflict Resolution** (⭐⭐⭐⭐)
   - Within each category, rules likely have **sub-weights**
   - Example: ATS Compatibility rules might prioritize:
     1. File format validation (highest priority)
     2. Section header validation
     3. Chronological order
     4. Date formatting
   - **No public documentation** of intra-category priorities

3. **LLM-Based Holistic Scoring** (⭐⭐⭐⭐)
   - Jobright uses LLM (GPT-4o/GPT-5) for analysis
   - LLM can make **context-aware judgments** that override mechanical rules
   - Example: A formatting rule might be ignored if it conflicts with a critical keyword match
   - **Interpretation**: LLM provides **tiebreaking** for edge cases

**Hypothesized Priority Resolution Algorithm:**

```python
# Hypothesized conflict resolution (High Confidence: 80%)

def resolve_rule_conflicts(rules, resume_data, job_description):
    # Step 1: Apply category weights (primary resolution)
    category_scores = {}
    for rule in rules:
        category = rule.category
        score = rule.check_function(resume_data, job_description)
...
**Conclusion:**
Jobright resolves rule conflicts through a **multi-layered approach**:

1. **Primary**: Category weights (40/25/20/10/5) resolve cross-category conflicts
2. **Secondary**: Sub-weights within categories (hypothesized, not confirmed)
3. **Tertiary**: LLM-based tiebreaking for ambiguous or edge cases
4. **Fallback**: First-match or default priority for intra-category conflicts without sub-weights

**Confidence Level: 80%** (Based on confirmed category weights + comparable implementation patterns)

**Testing Recommendations:**
- Create resume with deliberate rule conflicts (e.g., perfect ATS formatting vs. missing critical keywords)
- Observe which rule takes precedence
- Test across different job types to detect category-specific behaviors
...
### Rulesets: 14/15 Questions Answered (93%)
...
| Custom Rules        | ✅      | 85%        | **NO** - Not available          |
| Localization        | ❌      | 15%        | Unknown                         |
| Rule Priority       | ✅      | 80%        | Category weights + LLM tiebreak |
...
| -------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------ | ----------- |
| **tedix.dev MCP Listing**  | [https://tedix.dev/apps/resume-builder/](https://tedix.dev/apps/resume-builder/)             | Model stack, tool definitions, performance | ⭐⭐⭐⭐⭐       |
| **zPlatform Review**       | [https://zplatform.ai/ai-reviews/jobright-ai/](https://zplatform.ai/ai-reviews/jobright-ai/) | Matching accuracy, data sources, features  | ⭐⭐⭐⭐⭐       |
| **Jobright Official Site** | [https://jobright.ai](https://jobright.ai)                                                   | Database scale, geographic coverage        | ⭐⭐⭐⭐⭐       |
| **Jobright Blog**          | [https://jobright.ai/blog](https://jobright.ai/blog)                                         | ATS details, training data, workflow       | ⭐⭐⭐⭐⭐       |
| **AutoGPT Review**         | [https://autogpt.net/jobright-ai](https://autogpt.net/jobright-ai)...                        | Match scores, user testing                 | ⭐⭐⭐⭐⭐       |
...
| Source                 | URL                                                                                                            | Contributions                | Reliability |
| ---------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------- | ----------- |
| **ZeroSkillAI Review** | [https://zeroskillai.com/jobright-ai-review/](https://zeroskillai.com/jobright-ai-review/)                     | Feature analysis, ghost jobs | ⭐⭐⭐⭐        |
| **GoEnhance Blog**     | [https://www.goenhance.ai/blog/jobright-ai](https://www.goenhance.ai/blog/jobright-ai)                         | Algorithm overview           | ⭐⭐⭐⭐        |
| **OreateAI Blog**      | [https://www.oreateai.com/blog/jobright-ai](https://www.oreateai.com/blog/jobright-ai)...                      | Workflow, differentiation    | ⭐⭐⭐⭐        |
| **FastMCP Docs**       | [https://gofastmcp.com](https://gofastmcp.com)                                                                 | Framework implementation     | ⭐⭐⭐⭐        |
| **MCP Specification**  | [https://github.com/modelcontextprotocol/specification](https://github.com/modelcontextprotocol/specification) | Protocol standards           | ⭐⭐⭐⭐        |
...
| Source                | URL                                                                                      | Contributions      | Reliability |
| --------------------- | ---------------------------------------------------------------------------------------- | ------------------ | ----------- |
| **Hirecarta Review**  | [https://hirecarta.com/blog/jobright-review](https://hirecarta.com/blog/jobright-review) | Critical analysis  | ⭐⭐⭐         |
| **Red Hat Developer** | [https://developers.redhat.com/](https://developers.redhat.com/)...                      | MCP implementation | ⭐⭐⭐         |
| **FreeCodeCamp**      | [https://www.freecodecamp.org/news/](https://www.freecodecamp.org/news/)...              | MCP development    | ⭐⭐⭐         |
