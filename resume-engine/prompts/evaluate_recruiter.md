...
- `other` -- any other explicit, non-negotiable disqualifier (e.g. a state-hiring exclusion list) -- **not** a substitute for `field_domain`, `years_experience`, or `degree` when one of those is the better fit. `direction` is always `n/a`.
