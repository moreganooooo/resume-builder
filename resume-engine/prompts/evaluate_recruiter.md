...
- `years_experience` -- a stated years-of-experience number or range. Only flag this from an explicit figure stated in the JD's own text -- **never** from a title's implied seniority (e.g. "VP," "Director," "Senior," "Principal") when no number is stated; if you cannot quote an actual years-of-experience figure from the posting, this is not a `years_experience` blocker. Set `direction` to `under_qualified` when the candidate's own tenure, per their resume, falls *below* the stated floor -- this is the real disqualifier. Set it to `over_qualified` when the candidate clearly exceeds a low/entry-level floor ("0-2 years," "1-3 years") and the posting reads as junior relative to their background -- still worth recording (a recruiter may screen out an overqualified candidate too), but it is a different concern from not meeting the bar, so it must be tagged distinctly rather than treated the same way. When a years figure is itself qualified by a specific industry ("5 years in the drilling industry," "3+ years in healthcare"), categorize it as `field_domain` instead -- the disqualifying element is the industry match, not the raw tenure count. **`direction` is REQUIRED on every `years_experience` entry.** Emit it explicitly as `under_qualified` or `over_qualified` -- never omit the field, and never send `null`. Downstream code cannot distinguish an entry with no direction from one where the candidate falls below the bar, so leaving it off silently converts an over-qualification note into a disqualifier.
- `degree` -- a required academic degree, at a level the candidate does not hold, that the posting states as an actual requirement. Judge the degree LEVEL first: a posting requiring a Master's or PhD the candidate lacks is a real `degree` blocker. Do **not** flag a field-of-study mismatch against an open-ended field list -- "in statistics, mathematics, economics, or other quantitative disciplines" and "in Computer Science or a related technical field" are satisfied by any reasonably adjacent field the candidate holds, and a chemistry or physics degree is a quantitative discipline. Never emit this category for: (1) a GPA, class rank, or other academic-performance standard -- a GPA is not a degree; (2) the prestige or reputation of a school or program ("a high GPA from a well-respected Computer Science program", "a top-tier university"); (3) a degree listed under "Preferred", "Desired", "Nice to have", or "Bonus" -- only a stated requirement counts, and an advanced degree named as desirable alongside a required Bachelor's is not a blocker; (4) a degree the candidate already holds at the required level. When a posting offers alternative paths ("Master's plus 3 years, OR Bachelor's plus 5 years"), the candidate is blocked only if they satisfy NONE of the branches -- and when they hold the degree a branch asks for but fall short of that branch's years, the blocker is `years_experience`, not `degree`. `direction` is always `n/a` here.
