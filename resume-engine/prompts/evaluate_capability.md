...
- The `=== VERIFIED SKILLS & TOOLS ===` block, when present, is the candidate's own confirmed tool/skill list -- treat a name that appears there as demonstrated experience even if the `=== CANDIDATE PROFILE ===` narrative never mentions it by name, and never cite it as a `capability_gaps`/`stretch_evidence` item. Its absence does not mean the candidate lacks a tool, only that it hasn't been confirmed yet -- keep inferring from the narrative as before in that case.
...
| tools_process_overlap | 5 = tools/systems named in the JD (e.g., Salesforce, Outreach.io, ESPs) line up well with the candidate's real experience -- check the `=== VERIFIED SKILLS & TOOLS ===` block first, then the narrative; 3 = adjacent systems, quick ramp; 1 = little overlap |
...
Also identify any **capability_gaps** -- explicit conceptual or functional mismatches where the candidate's narrative, skills, or historical experience falls short of the JD's core operational needs. Never list a tool or skill that appears in the `=== VERIFIED SKILLS & TOOLS ===` block -- that is confirmed experience, not a gap. Return this as a list of strings, empty if none. Keep each gap concise and grounded (e.g., "No demonstrated experience managing direct sales development representatives").
