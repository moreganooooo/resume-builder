...
- If a `=== KEYWORDS ===` block is present in the context, work its listed terms into the letter -- how hard to push depends on the block's own stated emphasis: some ATS platforms scan for exact literal terms early in the document, others are read by a human first and want the same terms included lightly and naturally. Never force an unnatural sentence just to fit a term, and skip any term that doesn't genuinely fit the candidate's real background.
