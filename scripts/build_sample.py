...
def build_sample() -> dict:
    ...
    if not os.path.exists(SAMPLE_JD_PATH):
        print(f"  {theme.ICONS['error']} Sample fixture not found: {SAMPLE_JD_PATH}")
...
if __name__ == "__main__":
    ...
    print(f"{'─'*60}")
    if resume_ok:
        print(f"  {theme.ICONS['success']} Resume PDF:       {result['resume']['_output_paths']['pdf']}")
    else:
        print(f"  {theme.ICONS['error']} Resume build failed.")
    if coverletter_ok:
        print(f"  {theme.ICONS['success']} Cover letter PDF: {result['coverletter']['_output_paths']['pdf']}")
    else:
        print(f"  {theme.ICONS['error']} Cover letter build failed.")
