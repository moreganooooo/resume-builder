...
def _personal_narrative_reminder_text() -> str:
    """Nudges toward Settings & Upkeep -> Personal Narrative & Story
    whenever any of bootstrap_profile.PERSONAL_NARRATIVE_FIELDS is still
    blank. Unlike the skill-gap reminder above, these fields are never
    auto-generated by anything in the pipeline -- only the candidate can
    supply them (see bootstrap_profile's own comment on why) -- so
    nothing else will ever prompt for them once bootstrap is done; the
    banner is the only place a returning user gets reminded at all.
    Returns "" when a reminder isn't warranted, so callers can append
    unconditionally."""
    try:
        import os
        ...
        # Same reasoning as the skill-gap reminder: a profile with no
        # knowledge base yet has nothing to remind about, and this is
        # exactly the state a fresh clone's first launch is in.
        if not os.path.isdir(profile_paths.kb_dir()):
            return ""
    ...
    if not blank:
        return ""
    return (
        f"\n{theme.colorize_icon('hint')}  {len(blank)} personal narrative "
        "field(s) still blank (only you can fill these in -- Headline, "
        "Superpowers, etc.) -- add them from Settings & Upkeep for a "
        "stronger evaluation and cover letter."
    )
...
def display_main_banner(
    ...
    narrative_reminder = _personal_narrative_reminder_text()
    ...
    def render_frame(threshold):
        ...
        if narrative_reminder:
            body.append(Text.from_markup(narrative_reminder))
