...
def scan_csv(csv_path: Path) -> dict:
    ...
    if present_score_cols:
        ...
        all_blank_mask = (
            df[present_score_cols]
            .apply(lambda col: col.isna() | (col.astype(str).str.strip() == ""), axis=0)
            .all(axis=1)
        )
    else:
    ...
    if bullet_col and all_blank_mask.any():
        report["unscored_bullets"] = (
            df.loc[all_blank_mask, bullet_col].dropna().astype(str).str.strip().tolist()
        )
...
def print_report(report: dict) -> None:
    ...
    if "error" in report:
        cli_art.console.print(
            f"   {theme.colorize_icon('error')} Error: {report['error']}",
            soft_wrap=True,
        )
        return
    ...
    if report.get("note"):
        cli_art.console.print(
            f"   {theme.colorize_icon('warning')}  {report['note']}", soft_wrap=True
        )
    ...
    if report["missing_by_col"]:
        ...
        for col, count in report["missing_by_col"].items():
            ...
            cli_art.console.print(
                f"   {status} {escape(str(col))}: {count} blank", soft_wrap=True
            )
    else:
        cli_art.console.print(
            f"   {theme.colorize_icon('warning')}  No score columns present in this file.",
            soft_wrap=True,
        )

    if report["fully_unscored_rows"] == 0:
        cli_art.console.print(
            f"   {theme.colorize_icon('success')} All rows scored — nothing to do.",
            soft_wrap=True,
        )
    else:
        cli_art.console.print(
            f"   {theme.colorize_icon('evaluate')} Action needed: run score_keeper_gems.py to fill {report['fully_unscored_rows']} blank rows.",
            soft_wrap=True,
        )
...
def main():
    ...
    cli_art.console.print(
        f"{theme.colorize_icon('discovery')} detect_blank_scores.py — scanning for unscored bullets...",
        soft_wrap=True,
    )
    ...
    if total_unscored == 0:
        cli_art.console.print(
            f"{theme.colorize_icon('success')} All bullet bank files are fully scored. Nothing to do.",
            soft_wrap=True,
        )
    else:
        cli_art.console.print(
            f"{theme.colorize_icon('evaluate')} Run: python scripts/score_keeper_gems.py  to fill blank scores.",
            soft_wrap=True,
        )
    ...
    if args.fix:
        ...
        cli_art.console.print(
            f"\n{theme.colorize_icon('save')} Unscored bullets written to: {out_path} ({len(all_unscored)} rows)",
            soft_wrap=True,
        )
