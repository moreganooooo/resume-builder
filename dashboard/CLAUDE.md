...
- `dashboard/` is a native Go module leveraging the Charmbracelet ecosystem (Bubble Tea v2, Lip Gloss v2, Glamour Markdown, Harmonica physics, and Huh forms, launched via `resume dashboard`).
  ...
  - **VHS Recordings**: Tapes located in `dashboard/tapes/` (`menu`, `help`, `light` (Catppuccin Latte via `dashboard.py --theme`), `cli_menu` (Python CLI menu), `pipeline`, `pipeline_help`, `progress`, `progress_help`, `heatmap`, `jobs`, `jobs_help`, `jobs_search`, `jobs_filter` (workplace = Remote), `matrix`, `kb_view`, `kb_tools`, `kb_metrics`, `kb_facts`, `kb_projects` -- the KB screen has no `?` overlay, `answers`, `mobile`; run all via `scripts/capture_tui_visuals.py`, which writes `artifacts/tui_*.png|gif`. `answers` makes one real Gemini call) record animated GIFs via `vhs <tape_path>`.
