...
- `dashboard/` is a native Go module leveraging the Charmbracelet ecosystem (Bubble Tea v2, Lip Gloss v2, Glamour Markdown, Harmonica physics, and Huh forms, launched via `resume dashboard`).
  ...
  - **VHS Recordings**: Tapes located in `dashboard/tapes/` (`menu`, `help`, `light` (Catppuccin Latte via `dashboard.py --theme`), `cli_menu` (Python CLI menu), `pipeline`, `progress`, `heatmap`, `jobs`, `jobs_help`, `matrix`, `kb_view`, `kb_metrics`, `answers`, `mobile`; run all via `scripts/capture_tui_visuals.py`, which writes `artifacts/tui_*.png|gif`. `answers` makes one real Gemini call) record animated GIFs via `vhs <tape_path>`.
