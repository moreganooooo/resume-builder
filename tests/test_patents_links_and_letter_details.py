...
PATENTS = [
    {"title": "Method for Sensing a Thing", "number": "U.S. Patent 1,234,567 B2"}
]
RESUME = {
    "NAME": "Test Person",
    "TAGLINE": "ANALYST | REPORTING",
    "SUMMARY_TEXT": "Analyst.",
    "SKILLS": [],
    "EXPERIENCE": [],
    "EDUCATION": [],
}
...
def rendered(resume):
    with (
        ...
        patch.object(render_html, "_profile_layout", return_value="compact"),
    ):
...
class TestResumeParts(unittest.TestCase):
    def test_link_display_strips_scheme_www_and_slash(self):
        self.assertEqual(
            render_html.link_display("https://www.linkedin.com/in/x/"),
            "linkedin.com/in/x",
        )
        self.assertEqual(
            render_html.link_display("linkedin.com/in/y"), "linkedin.com/in/y"
        )
    ...
    def test_patents_section_only_when_present(self):
        ...
        self.assertNotIn(
            ">Patents<", rendered(dict(RESUME, PATENTS=[], HEADER_LINKS=[]))
        )
    ...
    def test_header_links_row_only_when_opted_in(self):
        ...
        self.assertEqual(
            rendered(dict(RESUME, HEADER_LINKS=[])).count('class="contact-row"'), 1
        )
...
class TestCoverLetterParts(unittest.TestCase):
    def test_first_address_line_is_bold(self):
        html = render_coverletter.build_recipient_block_html(
            "Acme", location="Austin, TX"
        )
        self.assertTrue(
            html.startswith(
                '<div class="letter-address"><strong>Acme Hiring Team</strong><br>'
            )
        )
    ...
    def test_role_title_bolded_once_in_first_paragraph_only(self):
        html = render_coverletter.build_body_paragraphs_html(
            [
                "As a data scientist applying for the Data Scientist role, I bring data.",
                "Data Scientist again.",
            ],
            "Data Scientist",
        )
    ...
    def test_role_title_check(self):
        letter = {
            "body_paragraphs": ["I am applying for the Senior Analyst role.", "More."]
        }
        self.assertEqual(
            validate_coverletter._check_role_title(letter, "Senior Analyst"), []
        )
        self.assertEqual(
            len(validate_coverletter._check_role_title(letter, "Data Engineer")), 1
        )
    ...
    def test_filler_lines_are_flagged(self):
        letter = {
            "body_paragraphs": [
                "I cut latency 30% at Acme. I thrive in high-stakes environments."
            ]
        }
        ...
        self.assertEqual(
            validate_coverletter._check_filler_lines(
                {"body_paragraphs": ["I cut latency 30%."]}
            ),
            [],
        )
...
class TestRoleTitle(unittest.TestCase):
    def test_posting_title_is_trimmed(self):
        self.assertEqual(
            orchestrator._coverletter_role_title(
                {"job_title": "Content Strategist (Remote)"}, "x"
            ),
            "Content Strategist",
        )
    ...
    def test_falls_back_to_resume_tagline_in_title_case(self):
        with patch.object(
            orchestrator,
            "_read_matching_resume_tagline",
            return_value="SENIOR DATA SCIENTIST | ML",
        ):
            self.assertEqual(
                orchestrator._coverletter_role_title({}, "x"), "Senior Data Scientist"
            )
        with patch.object(
            orchestrator,
            "_read_matching_resume_tagline",
            return_value="ML ENGINEER | X",
        ):
            self.assertEqual(
                orchestrator._coverletter_role_title({}, "x"), "ML Engineer"
            )
