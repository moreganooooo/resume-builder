...
class TestMergeCondensedBullets(unittest.TestCase):
    def test_only_targeted_bullets_change(self):
        ...
        merged = orchestrator._merge_condensed_bullets(original, condensed, {"long target"})
...
class TestCondenseTargets(unittest.TestCase):
    def test_longest_page1_bullets_only(self):
        resume = _resume(("Harbor Books", ["aa", "aaaa"]), ("Corner Bakery", ["a" * 50]))
        profile = {"roles": [{"name": "Harbor Books", "page": 1}, {"name": "Corner Bakery", "page": 2}]}
