...
PASSING_CRITIQUE = {
    ...
    "hidden_gem_reason": "Rare, specific outcome.",
}

FAILING_CRITIQUE = {
    "accuracy_score": 60,
    "believability_score": 55,
    "clarity_score": 70,
    "ats_value": 50,
    "manager_test": "FAIL",
    "weaknesses": "Vague and unquantified.",
    "hidden_gem_score": 10,
    "hidden_gem_flag": False,
    "hidden_gem_reason": "Nothing memorable.",
}
...
class TestQueueAcceptedRewrite(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.keepers_audited = os.path.join(
            self.tmpdir, "bullet-bank-keepers-audited.csv"
        )
    ...
    def test_queues_rewrite_that_clears_keeper_bar(self):
        queued = bullet_feedback.queue_accepted_rewrite(
            "Managed a team.",
            "Led a 12-person team to 3x pipeline growth.",
            "Acme",
            "[sales]",
            PASSING_CRITIQUE,
            path=self.needs_review,
        )
        self.assertTrue(queued)
        rows = self._rows()
        self.assertEqual(len(rows), 1)
        self.assertEqual(
            rows[0]["final_bullet"], "Led a 12-person team to 3x pipeline growth."
        )
        ...
        self.assertEqual(
            rows[0]["rewrite_status"], "", "must stay blank so triage routes it fresh"
        )

    def test_skips_rewrite_that_does_not_clear_keeper_bar(self):
        queued = bullet_feedback.queue_accepted_rewrite(
            "Managed a team.",
            "Helped manage a team somewhat.",
            "Acme",
            "[sales]",
            FAILING_CRITIQUE,
            path=self.needs_review,
    ...
    def test_skips_duplicate_already_in_keeper_bank(self):
        with open(self.keepers, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                {
                    "Bullet Point": "Led a 12-person team to 3x pipeline growth.",
                    "Role / Company": "Acme",
                }
            )
    ...
    def test_migrates_old_schema_file_forward_in_place(self):
        ...
        with open(self.needs_review, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                {
                    "Bullet Point": "An old pre-existing row.",
                    "Role / Company": "Legacy Co",
                }
            )
        ...
        self.assertEqual(
            rows[0]["hidden_gem_score"],
            "",
            "migrated old row should have blank new columns",
        )
        self.assertEqual(
            rows[1]["final_bullet"], "Led a 12-person team to 3x pipeline growth."
        )
