...
class TestDisplayMainBanner(unittest.TestCase):
    ...
    @patch("cli_art.os.path.isdir", return_value=True)
    @patch("cli_art.maintenance.get_last_run", return_value=None)
    @patch("cli_art.jd_manager.count_completed_resumes", return_value=1)
    @patch("picker.count_active_roles", return_value=2)
    @patch("picker.count_unevaluated_roles", return_value=0)
    def test_shows_skill_gap_scan_reminder_when_stale(
        self, mock_backlog, mock_pending, mock_completed, mock_get_last_run, mock_isdir
    ):
        console = Console(record=True, width=100, force_terminal=False)
        original = cli_art.console
        cli_art.console = console
...
class TestSkillGapScanReminderText(unittest.TestCase):
    @patch("cli_art.os.path.isdir", return_value=True)
    @patch("cli_art.maintenance.get_last_run", return_value=None)
    def test_never_run_returns_a_reminder(self, mock_get_last_run, mock_isdir):
        text = cli_art._skill_gap_scan_reminder_text()
        self.assertIn("Skill Gap Scan", text)

    @patch("cli_art.os.path.isdir", return_value=True)
    @patch("cli_art.maintenance.get_last_run")
    def test_recent_run_returns_empty(self, mock_get_last_run, mock_isdir):
        import datetime
    ...
    @patch("cli_art.os.path.isdir", return_value=True)
    @patch("cli_art.maintenance.get_last_run")
    def test_stale_run_returns_a_reminder(self, mock_get_last_run, mock_isdir):
        import datetime
    ...
    def test_unparseable_timestamp_returns_a_reminder(
        self, mock_get_last_run, mock_isdir
    ):
        text = cli_art._skill_gap_scan_reminder_text()
        self.assertIn("Skill Gap Scan", text)
