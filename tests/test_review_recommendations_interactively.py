...
class TestReviewRecommendationsInteractively(unittest.TestCase):
    ...
    def test_only_approved_recommendations_are_returned(self, mock_confirm):
        recs = [
            "Add a Salesforce bullet.",
            "Tighten the summary wording.",
            "Add a Tableau bullet.",
        ]
        checkpoint = {}
        mock_confirm.side_effect = _confirm_answers(True, False, True)
        approved = orchestrator._review_recommendations_interactively(
            recs, checkpoint, self.job_key
        )
        self.assertEqual(
            approved, ["Add a Salesforce bullet.", "Add a Tableau bullet."]
        )
    ...
    def test_approval_choices_are_checkpointed(self, mock_confirm):
        ...
        orchestrator._review_recommendations_interactively(
            recs, checkpoint, self.job_key
        )
        saved = jd_manager.load_checkpoint(self.job_key)
        self.assertEqual(
            saved["approved_recommendations"], ["Add a Salesforce bullet."]
        )
    ...
    @patch("orchestrator.questionary.confirm")
    def test_interrupted_review_declines_remaining_and_keeps_prior_approvals(
        self, mock_confirm
    ):
