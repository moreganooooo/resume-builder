...
class TestQuickstartCommand(unittest.TestCase):
    ...
    @patch("cli.doctor.run_checks", return_value=[_check("Python version", True, "3.13")])
    def test_all_checks_passing_prints_success_and_skips_api_key_prompt(self, mock_checks, mock_record):
    ...
    def test_missing_api_key_prompts_and_writes_it_then_rechecks(self, mock_write, mock_record):
        checks_missing = [_check("GEMINI_API_KEY", False, "not set", "Add it to .env")]
        checks_after = [_check("GEMINI_API_KEY", True, "set")]
        with patch("cli.doctor.run_checks", side_effect=[checks_missing, checks_after]) as mock_checks, \
        ...
        self.assertEqual(mock_checks.call_count, 2)  # initial check + re-check after writing

    @patch("cli.maintenance.record_run")
    def test_declining_the_api_key_prompt_does_not_write_or_crash(self, mock_record):
        checks_missing = [_check("GEMINI_API_KEY", False, "not set", "Add it to .env")]
        with patch("cli.doctor.run_checks", return_value=checks_missing), \
    ...
    @patch("cli.maintenance.record_run")
    def test_a_step_that_fails_and_stays_failed_is_reported_not_swallowed(self, mock_record):
