...
class TestMatchToTimeline(unittest.TestCase):
    ...
    def setUp(self):
        self.timeline = [
            bootstrap_timeline.TimelineEntry(
                company="Acme Corp",
                title="Sales Manager",
                start_date="2019",
                end_date="2022",
            ...
            bootstrap_timeline.TimelineEntry(
                company="Beta Inc", title="Analyst", start_date="2015", end_date="2018"
            ),
        ]
    ...
    def test_falls_back_to_misc_when_nothing_matches(self, mock_llm_match):
        achievement = RawAchievement(
            raw_text="did outbound sales work somewhere",
            company_hint=None,
            date_hint=None,
            title_hint=None,
            confidence="low",
    ...
    def test_llm_fallback_used_when_hints_dont_match(self, mock_llm_match):
        achievement = RawAchievement(
            raw_text="while doing outbound sales",
            company_hint=None,
            date_hint=None,
            title_hint=None,
            confidence="low",
    ...
    def test_dry_run_skips_llm_fallback(self):
        ...
        with patch("bootstrap_timeline._llm_match") as mock_llm_match:
            company, confidence = bootstrap_timeline.match_to_timeline(
                achievement, self.timeline, dry_run=True
            )
...
class TestLlmMatch(unittest.TestCase):
    ...
    def test_exact_match_returns_canonical_company(self, mock_parse, mock_generate):
        ...
        self.assertEqual(
            bootstrap_timeline._llm_match("did a thing", self.timeline), "Acme Corp"
        )

    @patch("bootstrap_timeline.GeminiClient.generate")
    @patch("bootstrap_timeline.GeminiClient.parse_json")
    def test_fuzzy_spelling_still_resolves_to_canonical_company(
        self, mock_parse, mock_generate
    ):
    ...
    @patch("bootstrap_timeline.GeminiClient.generate")
    @patch("bootstrap_timeline.GeminiClient.parse_json")
    def test_hallucinated_company_not_in_timeline_is_rejected(
        self, mock_parse, mock_generate
    ):
        mock_generate.return_value = ("{}", {})
        mock_parse.return_value = {
            "matched_company": "Some Other Company That Was Never Offered"
        }
