...
class TestVoiceMetrics(unittest.TestCase):
    ...
    def test_split_sentences_handles_abbreviations_and_decimals(self):
        ...
        self.assertEqual(
            sentences[0], "I grew revenue by 14.5% at Acme Inc. in St. Louis."
        )
        self.assertEqual(
            sentences[1], "For e.g., we built tools for Ph.D. researchers."
        )
    ...
    def test_uniform_ai_text_triggers_variance_violation(self):
        ...
        self.assertTrue(
            any("monotonous" in v.lower() or "std dev" in v.lower() for v in violations)
        )
    ...
    def test_type_token_ratio_flags_repetitive_text(self):
        ...
        self.assertTrue(
            any(
                "lexical diversity" in v.lower() or "type-token" in v.lower()
                for v in violations
            )
        )
    ...
    def test_consecutive_opener_detection(self):
        ...
        violations = voice_metrics.analyze_voice_metrics(letter_data)
        self.assertTrue(
            any("consecutive sentences begin with" in v.lower() for v in violations)
        )
    ...
    def test_custom_threshold_overrides_respected(self):
        ...
        violations = voice_metrics.analyze_voice_metrics(
            letter_data, rules=lenient_rules
        )
        self.assertEqual(violations, [])
