...
def _real_skills_widow_violation() -> str:
    ...
    return validate_resume._check_skills_line_lengths({"SKILLS": ["X" * 120]}, SKILLS_RULES)[0]


def _real_skills_third_line_violation() -> str:
    return validate_resume._check_skills_line_lengths({"SKILLS": ["X" * 230]}, SKILLS_RULES)[0]


def _real_bullet_widow_violation() -> str:
    # 112 chars, so it wraps at 108 and strands a 1-word widow.
    resume = {"EXPERIENCE": [{"achievements": ["Shipped " + "x" * 99 + " thing extra"]}]}
...
class TestRetryHintGuardsMatchRealValidatorOutput(unittest.TestCase):
    ...
    def test_skills_guard_matches_a_real_widow_violation(self):
        self.assertTrue(orchestrator._is_skills_line_violation(_real_skills_widow_violation()))

    def test_skills_guard_matches_a_real_third_line_violation(self):
        self.assertTrue(orchestrator._is_skills_line_violation(_real_skills_third_line_violation()))

    def test_bullet_guard_matches_a_real_bullet_widow_violation(self):
        self.assertTrue(orchestrator._is_bullet_widow_violation(_real_bullet_widow_violation()))

    def test_skills_guard_ignores_unrelated_violations(self):
        self.assertFalse(orchestrator._is_skills_line_violation("Bullet is 300 chars and exceeds"))
...
class TestMetricInventoryGate(unittest.TestCase):
    ...
    def test_fires_on_an_existing_metric_violation(self):
        self.assertTrue(orchestrator._needs_metric_inventory(["Metric '100' is used in 2 bullets"]))

    def test_does_not_fire_on_a_widow_violation_alone(self):
        self.assertFalse(orchestrator._needs_metric_inventory([_real_skills_widow_violation()]))
        self.assertFalse(orchestrator._needs_metric_inventory([_real_bullet_widow_violation()]))

    def test_does_not_fire_on_unrelated_violations(self):
        self.assertFalse(orchestrator._needs_metric_inventory(["Pronoun 'I' found outside the Why section"]))
