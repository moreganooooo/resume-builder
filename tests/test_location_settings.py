...
class TestReadWrite(_TempFilters):
    ...
    def test_comments_are_preserved(self):
        ...
        ls.write_settings({"zip": "62704", "radius_miles": 25}, self.path)
        body = self.read()
    ...
    def test_rewrite_replaces_rather_than_appends(self):
        ls.write_settings({"zip": "62704", "radius_miles": 25}, self.path)
        ls.write_settings({"zip": "62704", "radius_miles": 5}, self.path)
    ...
    def test_write_when_no_location_filter_anchor_exists(self):
        ...
        ls.write_settings({"zip": "62704", "radius_miles": 10}, path)
...
class TestValidateOrigin(unittest.TestCase):
    def test_valid_zip(self):
        self.assertTrue(ls.validate_origin("", "", "62704")[0])

    def test_valid_city_state(self):
        self.assertTrue(ls.validate_origin("Springfield", "NY", "")[0])
...
class TestDescribe(unittest.TestCase):
    ...
    def test_radius_summary(self):
        text = ls.describe(
            {"zip": "62704", "radius_miles": 15, "workplace_mode": "any"}
        )
        self.assertIn("62704", text)
        self.assertIn("15 mi", text)

    def test_remote_only_omits_radius(self):
        text = ls.describe(
            {"zip": "62704", "radius_miles": 15, "workplace_mode": "remote"}
        )
...
class TestMultiSelectRendering(_TempFilters):
    ...
    def test_single_item_list_renders_as_a_plain_string(self):
        ls.write_settings(
            {"zip": "62704", "radius_miles": 25, "workplace_mode": ["remote"]},
            self.path,
        )
    ...
    def test_empty_list_falls_back_to_any(self):
        ls.write_settings(
            {"zip": "62704", "radius_miles": 25, "workplace_mode": []}, self.path
        )
        self.assertEqual(ls.read_settings(self.path)["workplace_mode"], "any")

    def test_describe_joins_a_combination(self):
        text = ls.describe(
            {"zip": "62704", "radius_miles": 25, "workplace_mode": ["remote", "onsite"]}
        )
